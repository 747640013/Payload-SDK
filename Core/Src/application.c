...
static T_DjiReturnCode DjiUser_PrintConsole(const uint8_t *data,
                                            uint16_t dataLen);
...
// Attention: if you want to run payload sdk on extension port, please define
// the macro USE_USB_HOST_UART.
  ...
  if ((aircraftInfoBaseInfo.aircraftType == DJI_AIRCRAFT_TYPE_M300_RTK ||
    ...
    if (aircraftInfoBaseInfo.djiAdapterType ==
            DJI_SDK_ADAPTER_TYPE_SKYPORT_V2 ||
        aircraftInfoBaseInfo.djiAdapterType == DJI_SDK_ADAPTER_TYPE_NONE) {
  ...
  if (DjiTest_TimeSyncRegHandler(&testTimeSyncHandler) !=
      DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
  ...
  if (((aircraftInfoBaseInfo.aircraftType == DJI_AIRCRAFT_TYPE_M300_RTK ||
       ...
       aircraftInfoBaseInfo.mountPosition !=
           DJI_MOUNT_POSITION_TYPE_EXTENSION_PORT) ||
  ...
  T_DjiTestUpgradePlatformOpt stm32UpgradePlatformOpt = {
      ...
      .cleanUpgradeProgramFileStoreArea =
          DjiUpgradePlatformStm32_CleanUpgradeProgramFileStoreArea,
      .createUpgradeProgramFile =
          DjiUpgradePlatformStm32_CreateUpgradeProgramFile,
      .writeUpgradeProgramFile =
          DjiUpgradePlatformStm32_WriteUpgradeProgramFile,
      .readUpgradeProgramFile = DjiUpgradePlatformStm32_ReadUpgradeProgramFile,
      .closeUpgradeProgramFile =
          DjiUpgradePlatformStm32_CloseUpgradeProgramFile,
      ...
      .cleanUpgradeRebootState =
          DjiUpgradePlatformStm32_CleanUpgradeRebootState,
  };
  ...
  if (DjiTest_UpgradeStartService(&stm32UpgradePlatformOpt,
                                  testUpgradeConfig) !=
      DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
...
void DjiUser_MonitorTask(void const *argument) {
  ...
  while (1) {
    ...
    USER_LOG_DEBUG("Uart1 read buffer state: countOfLostData %d, "
                   "maxUsedCapacityOfBuffer %d.",
                   readBufferState.countOfLostData,
                   readBufferState.maxUsedCapacityOfBuffer);
    USER_LOG_DEBUG("Uart1 write buffer state: countOfLostData %d, "
                   "maxUsedCapacityOfBuffer %d.",
                   writeBufferState.countOfLostData,
                   writeBufferState.maxUsedCapacityOfBuffer);
#endif

#ifdef USING_UART_PORT_2
    UART_GetBufferState(UART_NUM_2, &readBufferState, &writeBufferState);
    USER_LOG_DEBUG("Uart2 read buffer state: countOfLostData %d, "
                   "maxUsedCapacityOfBuffer %d.",
                   readBufferState.countOfLostData,
                   readBufferState.maxUsedCapacityOfBuffer);
    USER_LOG_DEBUG("Uart2 write buffer state: countOfLostData %d, "
                   "maxUsedCapacityOfBuffer %d.",
                   writeBufferState.countOfLostData,
                   writeBufferState.maxUsedCapacityOfBuffer);
#endif
    ...
    USER_LOG_DEBUG("Uart3 read buffer state: countOfLostData %d, "
                   "maxUsedCapacityOfBuffer %d.",
                   readBufferState.countOfLostData,
                   readBufferState.maxUsedCapacityOfBuffer);
    USER_LOG_DEBUG("Uart3 write buffer state: countOfLostData %d, "
                   "maxUsedCapacityOfBuffer %d.",
                   writeBufferState.countOfLostData,
                   writeBufferState.maxUsedCapacityOfBuffer);
#endif
    ...
    // Attention: report system performance part is not intended for normal
    // application runtime use but as a debug aid.
    if (USER_UTIL_IS_WORK_TURN(runIndicateTaskStep++,
                               RUN_INDICATE_TASK_FREQ_0D1HZ,
                               RUN_INDICATE_TASK_FREQ_1HZ)) {
#if (configUSE_TRACE_FACILITY == 1)
      currentTaskStatusArraySize = uxTaskGetNumberOfTasks();
      currentTaskStatusArray = osalHandler->Malloc(currentTaskStatusArraySize *
                                                   sizeof(TaskStatus_t));
      if (currentTaskStatusArray == NULL) {
        continue;
      }

      currentTaskStatusArraySize = uxTaskGetSystemState(
          currentTaskStatusArray, currentTaskStatusArraySize, NULL);
      ...
      for (i = 0; i < currentTaskStatusArraySize; i++) {
        ...
        for (j = 0; j < lastTaskStatusArraySize; ++j) {
          if (currentTaskStatusArray[i].xTaskNumber ==
              lastTaskStatusArray[j].xTaskNumber) {
            cpuOccupyPercentage = (currentTaskStatusArray[i].ulRunTimeCounter -
                                   lastTaskStatusArray[j].ulRunTimeCounter) /
                                  configTICK_RATE_HZ /
                                  RUN_INDICATE_TASK_FREQ_0D1HZ;
            break;
          }
        ...
        USER_LOG_DEBUG(
            "%-16s\t%u\t%u\t%u", currentTaskStatusArray[i].pcTaskName,
            cpuOccupyPercentage,
...
static T_DjiReturnCode DjiUser_FillInUserInfo(T_DjiUserInfo *userInfo) {
  ...
  if (!strcmp(USER_APP_NAME, "your_app_name") ||
    ...
    USER_LOG_ERROR("Please fill in correct user information to "
                   "'samples/sample_c/platform/rtos_freertos/stm32f4_discovery/"
                   "application/dji_sdk_app_info.h' file.");
    return DJI_ERROR_SYSTEM_MODULE_CODE_INVALID_PARAMETER;
  }

  strncpy(userInfo->appName, USER_APP_NAME, sizeof(userInfo->appName) - 1);
  memcpy(userInfo->appId, USER_APP_ID,
         USER_UTIL_MIN(sizeof(userInfo->appId), strlen(USER_APP_ID)));
  memcpy(userInfo->appKey, USER_APP_KEY,
         USER_UTIL_MIN(sizeof(userInfo->appKey), strlen(USER_APP_KEY)));
  memcpy(userInfo->appLicense, USER_APP_LICENSE,
         USER_UTIL_MIN(sizeof(userInfo->appLicense), strlen(USER_APP_LICENSE)));
  memcpy(userInfo->baudRate, USER_BAUD_RATE,
         USER_UTIL_MIN(sizeof(userInfo->baudRate), strlen(USER_BAUD_RATE)));
  strncpy(userInfo->developerAccount, USER_DEVELOPER_ACCOUNT,
